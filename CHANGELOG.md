...
- Preserve direct-association `where:` metadata in Electric relationship
  subqueries.
...
- Resolve test repos, example apps and embedded Electric from one `DATABASE_URL`,
  retaining the test host and port variables as a fallback.
...
- Remove an unreachable disabled-mode child configuration clause.
